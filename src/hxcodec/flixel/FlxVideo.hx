...
class FlxVideo extends Video
{
	...
	override public function load(location:String, shouldLoop:Bool = false):Int
	{
		if (FlxG.autoPause)
		{
		...
		if (FileSystem.exists(Sys.getCwd() + location))
			return super.load(Sys.getCwd() + location, shouldLoop);
		else
			return super.load(location, shouldLoop);
	}
